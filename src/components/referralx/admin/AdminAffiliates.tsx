...
import { Users, UserPlus, UserCheck, UserX, Download, Search, Plus, Phone, Upload, FileDown, Trash2, CheckSquare, Pencil, ChevronLeft, ChevronRight } from "lucide-react";
...
interface AffiliatesResponse {
  ...
  page: number;
  limit: number;
  counts?: { all: number; active: number; inactive: number; withEarnings: number };
}
...
export function AdminAffiliates() {
  ...
  }, [token, search, statusFilter, page, perPage]);
  ...
  const openEdit = (a: Affiliate) => {
    setEditTarget(a);
    setEditForm({ name: a.User?.name || "", email: a.User?.email || "", phone: a.User?.phone || "" });
    setEditError(null);
  };
  ...
  const handleEditSave = async () => {
    ...
    setEditError(null);
    try {
      const res = await fetch(`/api/admin/affiliates/${editTarget.id}`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ name: editForm.name.trim(), email: editForm.email.trim(), phone: editForm.phone.trim() }),
      });
      const json = await res.json();
  ...
  const counts = data?.counts || { all: total, active: 0, inactive: 0, withEarnings: 0 };
  const activeCount = counts.active;
  const withEarningsCount = counts.withEarnings;
    ...
    <div className="space-y-6">
      ...
      {editTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-semibold text-rx-gray-800">Edit Ambassador</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-rx-gray-500 mb-1">Email <span className="text-rx-gray-400 font-normal">(also their login)</span></label>
